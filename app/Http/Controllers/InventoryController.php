...
class InventoryController extends Controller
{
    ...
    public function store(Request $request): JsonResponse
    {
        ...
        DB::beginTransaction();
        try {
            $doc = $this->insertInventory($data);           // create header + rows
            DB::commit();
    ...
    public function update(Request $request, Document $document): JsonResponse
    {
        ...
        try {
            $data = $this->validatePayload($request, $document->id);   // update-mode
        } catch (ValidationException $e) {
            return response()->json(['success' => false, 'errors' => $e->errors()], 422);
        }
        ...
        DB::beginTransaction();
        try {
            /* 1. откат прошлой инвентаризации */
    ...
    protected function validatePayload(Request $r, ?int $docId = null): array
    {
        return $r->validate([
            ...
            'products.*.qty'                            => ['required', 'numeric', 'gte:0'],   // ← только qty
        ]);
    }
    ...
    protected function insertInventory(array $data, ?Document $existing = null): Document
    {
        ...
        foreach ($data['products'] as $row) {
            ...
            $counted  = (float) $row['qty'];                           // ← qty вместо counted
    ...
    /** Отменяем влияние инвентаризации на склад */
    protected function rollbackInventory(Document $doc): void
    {
        ...
        foreach ($doc->items as $it) {
            ...
            if ($stock) {
                $stock->quantity = $it->quantity;   // возвращаем прежнее количество
                $stock->save();
            }
        }
    }
