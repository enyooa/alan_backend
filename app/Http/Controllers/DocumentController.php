...
public function updateIncomes(Request $request, Document $document): JsonResponse
{
    ...
    $validated = $request->validate([
        ...
        'products.*.product_subcard_id'              => ['required','uuid','exists:product_sub_cards,id'],
        'products.*.unit_measurement'                => ['required','string'],
        ...
        'expenses.*.provider_id'     => ['nullable','uuid','exists:providers,id'],
        'comments'                   => ['nullable','string'],
    ]);

    $whId     = $validated['warehouse_id'];
    ...
    DB::beginTransaction();
    try {
        /* 2-A. Возвращаем на склад ВСЁ, что было добавлено ранее --- */
        foreach ($document->items as $row) {
            ...
            if ($wh) {
                ...
                $wh->brutto      -= $row->brutto;
                $wh->netto       -= $row->netto;
                $wh->total_sum   -= $row->total_sum;
                $wh->save();
            }
        }

        /* 2-B. Очищаем старые строки и расходы -------------------- */
        $document->items()->delete();
        $document->expenses()->delete();

        /* 2-C. Обновляем «шапку» ---------------------------------- */
        $document->update([
            'provider_id'     => $validated['provider_id']  ?? $document->provider_id,
            'document_date'   => $validated['document_date'] ?? $document->document_date,
            'to_warehouse_id' => $whId,
            'comments'        => $validated['comments'] ?? $document->comments,
        ]);

        /* 2-D. Добавляем НОВЫЕ товары и двигаем склад ------------- */
        foreach ($products as $p) {
            $docItem = $document->items()->create([
                ...
                'net_unit_weight'     => $p['quantity']>0 ? round(($p['netto']??0)/$p['quantity'],4) : 0,
            ]);

            $wh = WarehouseItem::firstOrNew([
                    'warehouse_id'       => $whId,
                    'product_subcard_id' => $docItem->product_subcard_id,
                    'unit_measurement'   => $docItem->unit_measurement,
                 ]);

            $wh->quantity  += $docItem->quantity;
            $wh->brutto    += $docItem->brutto;
            $wh->netto     += $docItem->netto;
            $wh->total_sum += $docItem->total_sum;
            $wh->price      = $docItem->price;
            $wh->save();
        }

        /* 2-E. Добавляем (пере-)расходы --------------------------- */
...
/*=====================================================================
 |  DELETE  /api/income-products/{document}
 |  Полное удаление «Прихода» и возврат товара со склада
 *====================================================================*/
public function destroyIncomes(Document $document): JsonResponse
{
    ...
    DB::beginTransaction();
    try {
        /* 1. Вернём товар со склада-получателя ------------------------ */
        foreach ($document->items as $row) {
