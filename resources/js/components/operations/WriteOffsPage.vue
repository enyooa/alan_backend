<template>
  <div class="expense-page">
    ...
    <!-- create modal (используем ту же страницу, что и при редактировании) -->
    <div v-if="showCreate" class="modal-overlay">
      <div class="modal-container">
        <button class="close-btn" @click="showCreate = false">×</button>
        <EditWriteOffModal @saved="onSaved" @close="showCreate = false" />
      </div>
    </div>
...
import EditWriteOffModal from '../../views/forms/products/EditWriteOffModal.vue'
import ModalShell        from '../../views/forms/products/ModalShell.vue'

export default {
  name: 'WriteOffsPage',
  components: { EditWriteOffModal, ModalShell },
  ...
  methods: {
    ...
    async load () {
      try {
        const { data } = await axios.get('/api/writeoff-products')
        this.raw = Array.isArray(data) ? data : []
        this.applyFilter()
      } catch (e) { console.error(e); alert('Не удалось загрузить списания') }
    },
