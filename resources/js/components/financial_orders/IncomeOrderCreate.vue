<template>
    <div class="page">
      ...
      <header class="appbar">
        ...
        <span class="title">Создать приходный ордер</span>
      ...
      <section class="card">
        ...
        <FieldRow label="Контрагент">
          <select v-model="form.counterparty">
            ...
            <optgroup label="Клиенты">
              <option v-for="c in parties.clients"
                      :key="c.id"
                      :value="toVal(c,'client')">
                {{ c.name }}
              </option>
            </optgroup>
        ...
        <FieldRow label="Статья движения">
    ...
    methods:{
      async fetchRefs(){
        try{
          ...
          this.parties.clients   = p.data.filter(x=>x.type==='client')
...
.fab{
  ...
  .fab:hover:not(.disabled){
    filter:brightness(1.08);transform:translate(-50%,-3px);
  }

  /* состояние «saving / disabled» */
  .fab.disabled,
  .fab:disabled{
    /* вместо прозрачности — другой фон */
    background:#9ddde7;             /* мягкий голубой */
    box-shadow:none;                /* убираем тень, чтобы кнопка "приглушилась" */
    cursor:progress;
    transform:translateX(-50%);      /* сбросим возможный hover-сдвиг */
  }
