<!-- resources/js/components/financial_orders/IncomeOrderCreate.vue -->
<template>
  <div class="page">
    <!-- ▸ APP-BAR (fixed) -->
    <header class="appbar">
    ...
    <div class="page-scroll">
      ...
      <section class="card">
        ...
        <FieldRow label="Счёт кассы">
          <select v-model="form.cashId" required>
            <option disabled value="">Выберите…</option>
            <option v-for="c in cashes" :key="c.id" :value="c.id">
              {{ c.name }}
            </option>
          </select>
        </FieldRow>
  ...
  methods: {
    ...
    async send() {
      ...
      try {
        ...
        this.$emit('created') // уведомляем родителя
      } catch (e) {
        console.error(e)
